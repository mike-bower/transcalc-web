(************************************************)
(*   Non Linear Poisson Bridge Unit             *)
(************************************************)
unit Nonline;
...
(************************************************)
(*   TNonLinearPoisson Object                   *)
(************************************************)
(*
type TNonLinearPoisson = class(TLogItem)
private
  FStrain          : string;
  FGageFactor      : string;
  FPoisson         : string;
...
(************************************************)
(*   TNonLinearForm Object                      *)
(************************************************)
type
  TNonLinearForm = class(TForm)
    BN_OK: TBitBtn;
    BN_Cancel: TBitBtn;
    Panel1: TPanel;
    Image1: TImage;
  ...
  private
    ...
    procedure PrintForm;
  public
{    NonLinearPoissonObject  : TNonLinearPoisson;}
    NonLinearPoissonChosen  : boolean;
  end;

var
  NonLinearForm: TNonLinearForm;
...
(***********************************************)
(*  NonLinear Poisson Object Methods           *)
(***********************************************)

(***********************************************************************)
{TNonLinearPoisson.create}
(*
constructor TNonLinearPoisson.create;
begin
  {...creating the object}
  inherited create;
  {...setting the objects stream so know what kind of object it is when save}
  TheString := '2 + 2v Arms, Axial/Bending'
end;
*)

(***********************************************************************)
{TNonLinearPoisson.destroy}
(*
destructor TNonLinearPoisson.destroy;
begin
  {...destroying object}
  inherited destroy;
end;
...
(***********************************************************************)
{TNonLinearPoisson.GetAsString}
(*
function TNonLinearPoisson.GetAsString: string;
begin
  Result := TheString;
end;
*)
...
(***********************************************************************)
{TNonLinearPoisson.ShowTheForm}
(*
procedure TNonLinearPoisson.ShowTheForm;
begin
  Application.CreateForm(TNonLinearForm, NonLinearForm);
  NonLinearForm.showmodal;
end;
*)

(***********************************************************************)
{TNonLinearPoisson.AssignTo}
(*
procedure TNonLinearPoisson.AssignTo(Dest: TPersistent);
begin
  if (Dest is TNonLinearPoisson) and (Self is Dest.ClassType) then begin
    inherited AssignTo(TLogItem(Dest));
    with Dest as TNonLinearPoisson do begin
...
(***********************************************************************)
{TNonLinearPoisson.InitFields}
(*
procedure TNonLinearPoisson.InitFields;
begin
...
(***********************************************************************)
{TNonLinearPoisson.SaveToStream}
(*
procedure TNonLinearPoisson.SaveToStream(Stream: TacObjStream);
begin
...
(***********************************************************************)
{TNonLinearPoisson.ReadFromStream}
(*
procedure TNonLinearPoisson.ReadFromStream(Stream: TacObjStream);
begin
...
(***********************************************)
(*  NonLinear Poisson Form Methods             *)
(***********************************************)

(***********************************************************************)
{TNonLinearForm.CalculateOutput}
function TNonLinearForm.CalculateOutput: RealType;
var
...
(***********************************************************************)
{TNonLinearForm.BN_ComputeClick}
procedure TNonLinearForm.BN_ComputeClick(Sender: TObject);
const
  MINSTRAIN     = -5000;
  MAXSTRAIN     = 5000;
  MINGAGEFACTOR = 1.00;
...
(***********************************************************************)
{TNonLinearForm.BN_CancelClick}
procedure TNonLinearForm.BN_CancelClick(Sender: TObject);
begin
  {...placing old variables in edit boxes}
  EB_Strain.text := Old_Strain;
  EB_GageFactor.text := Old_GageFactor;
  EB_Poisson.text := Old_Poisson;
  ST_Output.caption := Old_Output;
  if NonLinearPoissonChosen = false then begin
    NonLinearPoissonChosen := false;
...
(***********************************************************************)
{TNonLinearForm.FormCreate}
procedure TNonLinearForm.FormCreate(Sender: TObject);
begin
...
(***********************************************************************)
{TNonLinearForm.FormActivate}
procedure TNonLinearForm.FormActivate(Sender: TObject);
var
  EditStrings  : string;
  i            : integer;
...
(*
  ...
  i := 0;
  Repeat
    if MainForm.FLog.AtIndex(i) is TNonLinearPoisson then begin
      TheItem := MainForm.FLog.AtIndex(i);
    end;
    inc(i);
  until (TheItem <> nil) or (i > MainForm.Flog.Count);
  {...if the object exist, then placing object variables into edit boxes}
  if TheItem <> nil then begin
    EB_Strain.text := (TheItem as TNonLinearPoisson).Strain;
    EB_GageFactor.text := (TheItem as TNonLinearPoisson).GageFactor;
    EB_Poisson.text := (TheItem as TNonLinearPoisson).Poisson;
    ST_Output.caption := (TheItem as TNonLinearPoisson).BridgeOutput;
  end;
*)
  {...getting initial variables and placing in old variables in case cancel clicked}
...
(***********************************************************************)
{TNonLinearForm.BN_OKClick}
procedure TNonLinearForm.BN_OKClick(Sender: TObject);
(*
var
  TheItem : TACStreamable;
  i       : integer;
...
(*
  ...
  if TheItem = nil then begin
    ...
    NonLinearPoissonObject := TNonLinearPoisson.create;
    NonLinearPoissonObject.Strain := EB_Strain.text;
    NonLinearPoissonObject.GageFactor := EB_GageFactor.text;
    NonLinearPoissonObject.Poisson := EB_Poisson.text;
    NonLinearPoissonObject.BridgeOutput := ST_Output.caption;
    MainForm.AddToLog(NonLinearPoissonObject);
    j := 0;
    ...
    MainForm.XCalcOutline.AddChild(j, NonLinearPoissonObject.GetAsString);
  end
  else begin
    (TheItem as TNonLinearPoisson).Strain := EB_Strain.text;
    (TheItem as TNonLinearPoisson).GageFactor := EB_GageFactor.text;
    (TheItem as TNonLinearPoisson).Poisson := EB_Poisson.text;
    (TheItem as TNonLinearPoisson).BridgeOutput := ST_Output.caption;
  end;
...
procedure TNonLinearForm.FormClose(Sender: TObject;
...
begin
  {...registering object}
  RegisterClasses([TNonLinearPoisson]);
*)

procedure TNonLinearForm.BN_PrintClick(Sender: TObject);
begin
...
{TNonLinearForm.GotFormImage}
function TNonLinearForm.GotFormImage: TBitmap;
var
...
(***********************************************************************)
{TNonLinearForm.PrintForm}
procedure TNonLinearForm.PrintForm;
var
  FormImage: TBitmap;
  Info: PBitmapInfo;
  InfoSize: Cardinal;
