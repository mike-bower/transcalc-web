...
(************************************************)
(*   TOneActiveOnePoisson Object                *)
(************************************************)
(*
type TOneActiveOnePoisson = class(TLogItem)
private
  FStrain          : string;
  FGageFactor      : string;
  FPoisson         : string;
...
(************************************************)
(*   TOneActiveOnePoisson Form Object           *)
(************************************************)
type
  TOneActiveOnePoissonFrm = class(TForm)
    BN_OK: TBitBtn;
    BN_Cancel: TBitBtn;
    Panel1: TPanel;
    Image1: TImage;
 ...
 private
  ...
  public
(*  OneActiveOnePoissonObject  : TOneActiveOnePoisson;
*)
    OneActiveOnePoissonChosen  : boolean;
  end;

var
  OneActiveOnePoissonFrm: TOneActiveOnePoissonFrm;
...
(***********************************************)
(*  One Active One Poisson Object Methods      *)
(***********************************************)

(***********************************************************************)
{TOneActiveOnePoisson.create}
(*
constructor TOneActiveOnePoisson.create;
begin
  {...creating object}
  inherited create;
  {...setting the string so know what kind of object it is}
  TheString := '1 + v Arms, Axial/Bending'
end;
*)

(***********************************************************************)
{TOneActiveOnePoisson.Destroy}
(*
destructor TOneActiveOnePoisson.Destroy;
begin
  {...destroying object}
  inherited destroy;
end;
...
(***********************************************************************)
{TOneActiveOnePoisson.GetAsString}
(*
function TOneActiveOnePoisson.GetAsString: string;
begin
  Result := TheString;
end;
*)
...
(***********************************************************************)
{TOneActiveOnePoisson.ShowTheForm}
(*
procedure TOneActiveOnePoisson.ShowTheForm;
begin
  Application.CreateForm(TOneActiveOnePoissonFrm, OneActiveOnePoissonFrm);
  OneActiveOnePoissonFrm.showmodal;
end;
*)

(***********************************************************************)
{TOneActiveOnePoisson.AssignTo}
(*
procedure TOneActiveOnePoisson.AssignTo(Dest: TPersistent);
begin
  if (Dest is TOneActiveOnePoisson) and (Self is Dest.ClassType) then begin
    inherited AssignTo(TLogItem(Dest));
    with Dest as TOneActiveOnePoisson do begin
...
(***********************************************************************)
{TOneActiveOnePoisson.InitFields}
(*
procedure TOneActiveOnePoisson.InitFields;
begin
...
(***********************************************************************)
{TOneActiveOnePoisson.SaveToStream}
(*
procedure TOneActiveOnePoisson.SaveToStream(Stream: TacObjStream);
begin
...
(***********************************************************************)
{TOneActiveOnePoisson.ReadFromStream}
(*
procedure TOneActiveOnePoisson.ReadFromStream(Stream: TacObjStream);
begin
...
(***********************************************)
(*  One Active One Poisson Form Methods        *)
(***********************************************)

(***********************************************************************)
{TOneActiveOnePoisson.CalculateOutput}
function TOneActiveOnePoissonFrm.CalculateOutput: RealType;
var
...
begin
  ...
  try
    Numerator := TheStrain - (-TheStrain * ThePoisson);
  except
...
(***********************************************************************)
{TOneActiveOnePoissonFrm.BN_ComputeClick}
procedure TOneActiveOnePoissonFrm.BN_ComputeClick(Sender: TObject);
const
  MINSTRAIN     = -5000;
  MAXSTRAIN     = 5000;
  MINGAGEFACTOR = 1.00;
...
begin
  ...
  if ExceptionRaised = false then begin
    TheOutput := floattostrf(OutputValue, ffFixed, 6, 3);
    ST_Output.caption := TheOutput;
  end;
end;

(***********************************************************************)
{TOneActiveOnePoissonFrm.BN_CancelClick}
procedure TOneActiveOnePoissonFrm.BN_CancelClick(Sender: TObject);
begin
  {...placing the old variables in the edit boxes}
  EB_Strain.text := Old_Strain;
  EB_GageFactor.text := Old_GageFactor;
  EB_Poisson.text := Old_Poisson;
  ST_Output.caption := Old_Output;
  if OneActiveOnePoissonChosen = false then begin
    OneActiveOnePoissonChosen := false;
...
(***********************************************************************)
{TOneActiveOnePoissonFrm.FormCreate}
procedure TOneActiveOnePoissonFrm.FormCreate(Sender: TObject);
begin
...
(***********************************************************************)
{TOneActiveOnePoissonFrm.FormActivate}
procedure TOneActiveOnePoissonFrm.FormActivate(Sender: TObject);
var
  EditStrings  : string;
  i            : integer;
...
(*
  ...
  i := 0;
  Repeat
    if MainForm.FLog.AtIndex(i) is TOneActiveOnePoisson then begin
      TheItem := MainForm.FLog.AtIndex(i);
    end;
    inc(i);
  until (TheItem <> nil) or (i > MainForm.Flog.Count);
  {If there is an object, the placing it in the EditBoxes}
  if TheItem <> nil then begin
    EB_Strain.text := (TheItem as TOneActiveOnePoisson).Strain;
    EB_GageFactor.text := (TheItem as TOneActiveOnePoisson).GageFactor;
    EB_Poisson.text := (TheItem as TOneActiveOnePoisson).Poisson;
    ST_Output.caption := (TheItem as TOneActiveOnePoisson).BridgeOutput;
  end;
  *)
  {...placing the Edit box data into old variables in case cancel clicked}
...
(***********************************************************************)
{TOneActiveOnePoissonFrm.BN_OKClick}
procedure TOneActiveOnePoissonFrm.BN_OKClick(Sender: TObject);
(*
var
  TheItem : TACStreamable;
  i       : integer;
...
*)
begin
  OneActiveOnePoissonChosen := true;
  ...
  {If there is an object, the placing it in the EditBoxes}
  if TheItem = nil then begin
    {...placing the values in object}
    OneActiveOnePoissonObject := TOneActiveOnePoisson.create;
    OneActiveOnePoissonObject.Strain := EB_Strain.text;
    OneActiveOnePoissonObject.GageFactor := EB_GageFactor.text;
    OneActiveOnePoissonObject.Poisson := EB_Poisson.text;
    OneActiveOnePoissonObject.BridgeOutput := ST_Output.caption;
    MainForm.AddToLog(OneActiveOnePoissonObject);
    j := 0;
    ...
    MainForm.XCalcOutline.AddChild(j, OneActiveOnePoissonObject.GetAsString);
  end
  else begin
    (TheItem as TOneActiveOnePoisson).Strain := EB_Strain.text;
    (TheItem as TOneActiveOnePoisson).GageFactor := EB_GageFactor.text;
    (TheItem as TOneActiveOnePoisson).Poisson := EB_Poisson.text;
    (TheItem as TOneActiveOnePoisson).BridgeOutput := ST_Output.caption;
  end;
...
procedure TOneActiveOnePoissonFrm.FormClose(Sender: TObject;
...
begin
  {...registering object}
  RegisterClasses([TOneActiveOnePoisson]);
*)
procedure TOneActiveOnePoissonFrm.BN_PrintClick(Sender: TObject);
begin
...
{TOneActiveOnePoissonFrm.GotFormImage}
function TOneActiveOnePoissonFrm.GotFormImage: TBitmap;
var
...
(***********************************************************************)
{TOneActiveOnePoissonFrm.PrintForm}
procedure TOneActiveOnePoissonFrm.PrintForm;
var
  FormImage: TBitmap;
  Info: PBitmapInfo;
  InfoSize: Cardinal;
