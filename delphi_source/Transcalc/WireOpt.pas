...
type
   TWire  = class(Tobject)
    ...
    TCR        : real;
    (*
    constructor init(aName: PChar; aResistivity, aTCR: real);
...
type
  ...
  public
    constructor create;
    destructor destroy;
    property SolveFor: string read FSolveFor write FSolveFor;
    property Resistance: string read FResistance write FResistance;
    property Length: string read FLength write FLength;
    property WireType: integer read FWireType write FWireType;
    property TheStringText: string read TheString write TheString;
end;

(***************************************************)
(*Span vs. Temperature Opt  Wire Table Form Object *)
(***************************************************)
type
  TWireTableOptForm = class(TForm)
    Panel1: TPanel;
    Image1: TImage;
    Panel2: TPanel;
    ST_Input: TLabel;
  ...
  public
    WireTableOptObject  : TOptWireTable;
    Wire                : TWire;
  end;

var
  WireTableOptForm: TWireTableOptForm;
...
(*constructor TWire.init(aName: PChar; aResistivity, aTCR: real);
begin
  TObject.create;
  StrCopy(Name, aName);
  Resistivity := aResistivity;
  TCR := aTCR;
end;
...
(***********************************************)
(*  Wire Table Object Methods                  *)
(***********************************************)

(***********************************************************************)
{TOptWireTable.create}
constructor TOptWireTable.create;
begin
  {...creating object}
  inherited create;
  {...associating string with object}
  TheString := 'Wire Table (Zero vs. Temperature)';
end;

(***********************************************************************)
{T2pTOptWireTable.destroy}
destructor TOptWireTable.destroy;
begin
  {...destroying object}
  inherited destroy;
end;
...
(***********************************************************************)
{TOptWireTable.GetAsString}
function TOptWireTable.GetAsString: string;
begin
  Result := TheString;
end;

(***********************************************************************)
{TOptWireTable.ShowTheForm}
procedure TOptWireTable.ShowTheForm;
begin
  Application.CreateForm(TWireTableOptForm, WireTableOptForm);
  WireTableOptForm.showmodal;
end;

(***********************************************************************)
{TOptWireTable.AssignTo}
procedure TOptWireTable.AssignTo(Dest: TPersistent);
begin
  if (Dest is TOptWireTable) and (Self is Dest.ClassType) then begin
    inherited AssignTo(TLogItem(Dest));
    with Dest as TOptWireTable do begin
      FSolveFor := self.FSolveFor;
...
(***********************************************************************)
{TOptWireTable.InitFields}
procedure TOptWireTable.InitFields;
begin
  inherited InitFields;
end;

(***********************************************************************)
{TOptWireTable.SaveToStream}
procedure TOptWireTable.SaveToStream(Stream: TacObjStream);
begin
  inherited SaveToStream(Stream);
  Stream.SaveBuffer(FSolveFor, sizeof(string));
  Stream.SaveBuffer(FResistance, sizeof(string));
  Stream.SaveBuffer(FLength, sizeof(string));
  Stream.SaveBuffer(FWireType, sizeof(integer));
  Stream.SaveBuffer(TheString, sizeof(string));
end;

(***********************************************************************)
{TOptWireTable.ReadFromStream}
procedure TOptWireTable.ReadFromStream(Stream: TacObjStream);
begin
  inherited ReadFromStream(Stream);
  Stream.ReadBuffer(FSolveFor, sizeof(string));
  Stream.ReadBuffer(FResistance, sizeof(string));
  Stream.ReadBuffer(FLength, sizeof(string));
  Stream.ReadBuffer(FWireType, sizeof(integer));
...
(***********************************************************************)
{TWireTableOptForm.RB_ResistanceClick}
procedure TWireTableOptForm.RB_ResistanceClick(Sender: TObject);
var
...
(***********************************************************************)
{TWireTableOptForm.RB_LengthClick}
procedure TWireTableOptForm.RB_LengthClick(Sender: TObject);
var
  LengthUnit   : array[0..MAXTEXTLENGTH-1] of char;
  OhmsUnit     : array[0..MAXTEXTLENGTH-1] of char;
begin
...
(***********************************************************************)
{TWireTableOptForm.FormShow}
procedure TWireTableOptForm.FormShow(Sender: TObject);
begin
...
(***********************************************************************)
{TWireTableOptForm.FormCreate}
procedure TWireTableOptForm.FormCreate(Sender: TObject);
var
  TheItem : string;
begin
...
(***********************************************************************)
{TWireTableOptForm.FormClose}
procedure TWireTableOptForm.FormClose(Sender: TObject;
...
(***********************************************************************)
{TWireTableOptForm.FormActivate}
procedure TWireTableOptForm.FormActivate(Sender: TObject);
var
  EditStrings  : string;
  i            : integer;
...
begin
  ...
  CB_WireType.items.addobject(TheItems, Wire);
  Repeat
    if MainForm.FLog.AtIndex(i) is TOptWireTable then begin
  ...
  if TheItem <> nil then begin
    SolveForText := (TheItem as TOptWireTable).SolveFor;
    if (SolveForText = 'Resistance') then begin
      RB_Resistance.checked := true;
      EB_Input.text := (TheItem as TOptWireTable).Length;
      ST_Answer.caption := (TheItem as TOptWireTable).Resistance;
    end
    else if (SolveForText =  'Length') then begin
      RB_Length.checked := true;
      EB_Input.text := (TheItem as TOptWireTable).Resistance;
      ST_Answer.caption := (TheItem as TOptWireTable).Length;
    end;
    TheType := (TheItem as TOptWireTable).WireType;
    CB_WireType.ItemIndex := TheType;
  end
  else begin
    CB_Wiretype.Itemindex := 0;
    RB_Length.checked := true;
...
(***********************************************************************)
{TWireTableOptForm.ReCalculate}
procedure TWireTableOptForm.ReCalculate;
const
  MINLENGTH_US = 0.01;
  MAXLENGTH_US = 100.0;
  MINRESIST    = 0.01;
...
(***********************************************************************)
{TWireTableOptForm.BN_ComputeClick}
procedure TWireTableOptForm.BN_ComputeClick(Sender: TObject);
begin
  ReCalculate;
end;

(***********************************************************************)
{TWireTableOptForm.BN_OKClick}
procedure TWireTableOptForm.BN_OKClick(Sender: TObject);
var
  i            : integer;
  TheItem      : TacStreamable;
begin
  ...
  {...placing the fields in the object}
    WireTableOptObject := TOptWireTable.create;
    WireTableOptObject.SolveFor := SolveForText;
    if (SolveForText = 'Resistance') then begin
      WireTableOptObject.Resistance := ST_Answer.caption;
      WireTableOptObject.Length := EB_Input.text;
    end
    else if (SolveForText = 'Length') then begin
      WireTableOptObject.Resistance := EB_Input.text;
      WireTableOptObject.Length := ST_Answer.caption;
    end;
    WireTableOptObject.WireType := TheType;
    MainForm.AddToLog(WireTableOptObject);
  end
  else begin
    (TheItem as TOptWireTable).SolveFor := SolveForText;
    if (SolveForText = 'Resistance') then begin
      (TheItem as TOptWireTable).Resistance := ST_Answer.caption;
      (TheItem as TOptWireTable).Length := EB_Input.text;
    end
    else if (SolveForText = 'Length') then begin
      (TheItem as TOptWireTable).Resistance := EB_Input.text;
      (TheItem as TOptWireTable).Length := ST_Answer.caption;
    end;
    (TheItem as TOptWireTable).WireType := TheType;
  end;
...
(***********************************************************************)
{TWireTableOptForm.CB_WireTypeChange}
procedure TWireTableOptForm.CB_WireTypeChange(Sender: TObject);
var
 TheStr    : string;
 TheObject : TObject;
 TheWire   : Tobject;
...
(***********************************************************************)
{TWireTableOptForm.WireGridSelectCell}
procedure TWireTableOptForm.WireGridSelectCell(Sender: TObject; Col,
