...
const
  ...
  (* ************************************************* *)
  (* Span Set Wire Table Form Object *)
  (* ************************************************* *)
type
  TWireTableSpanForm = class(TForm)
    Panel1: TPanel;
    Image1: TImage;
    Panel2: TPanel;
    ST_Input: TLabel;
...
var
  WireTableSpanForm: TWireTableSpanForm;

implementation

uses MainMenu, convert, SpanSet;
{$R *.DFM}
...
(* ********************************************************************* *)
{ TWireTableSpanForm.FormShow }
procedure TWireTableSpanForm.FormShow(Sender: TObject);
begin
...
procedure TWireTableSpanForm.FormCreate(Sender: TObject);
var
...
begin
  ...
  EB_Input.text := SpanSetForm.ST_Resistance.caption;
  ST_Answer.caption := '';
end;

(* ********************************************************************* *)
{ TWireTableSpanForm.FormClose }
procedure TWireTableSpanForm.FormClose(Sender: TObject;
...
(* ********************************************************************* *)
{ TWireTableSpanForm.FormActivate }
procedure TWireTableSpanForm.FormActivate(Sender: TObject);
begin
  ST_Type.caption := SpanSetForm.CB_WireType.text;
  CB_WireTypeChange(Sender);
  if ST_Type.caption = 'Constantan (A Alloy)' then
  begin
    WireGrid.Row := 1;
  end
  else if ST_Type.caption = 'Manganin' then
  begin
    WireGrid.Row := 8;
  end
  else if ST_Type.caption = 'Modified Karma (K Alloy)' then
  begin
    WireGrid.Row := 1;
  end;
end;
...
(* ********************************************************************* *)
{ TWireTableSpanForm.ReCalculate }
procedure TWireTableSpanForm.ReCalculate;
const
  MINLENGTH_US = 0.01;
  MAXLENGTH_US = 100.0;
  MINRESIST = 0.01;
...
begin
  ...
  else if (Resistance < MINRESIST) or (Resistance > MAXRESIST) then
  begin
    IsThereAnError := true;
...
(* ********************************************************************* *)
{ TWireTableSpanForm.BN_ComputeClick }
procedure TWireTableSpanForm.BN_ComputeClick(Sender: TObject);
begin
  ReCalculate;
end;

(* ********************************************************************* *)
{ TWireTableSpanForm.BN_OKClick }
procedure TWireTableSpanForm.BN_OKClick(Sender: TObject);
begin
...
(* ********************************************************************* *)
{ TWireTableSpanForm.CB_WireTypeChange }
procedure TWireTableSpanForm.CB_WireTypeChange(Sender: TObject);
var
  TheStr: string;
  TheObject: TObject;
  TheWire: TObject;
  ...
  j: integer;
begin
  TheType := SpanSetForm.CB_WireType.itemindex;
  TheItems := SpanSetForm.CB_WireType.Items;
  TheObject := TheItems.objects[SpanSetForm.CB_WireType.itemindex];
  WireID := TheItems.strings[SpanSetForm.CB_WireType.itemindex];
  TheObject := TheItems.objects[SpanSetForm.CB_WireType.Items.indexof(WireID)];
  { ...placing Diameter in string grid }
  for i := AWGlo to AWGhi do
  begin
    if MainForm.UnitsAreUS = true then
    begin
  ...
  begin
    ...
    WireGage := format('%10.0f', [table[i].AWG]);
...
(* ********************************************************************* *)
{ TWireTableSpanForm.WireGridSelectCell }
procedure TWireTableSpanForm.WireGridSelectCell(Sender: TObject; Col,
...
(* ********************************************************************* *)
{ TWireTableSpanForm.BN_HelpClick }
procedure TWireTableSpanForm.BN_HelpClick(Sender: TObject);
begin
  Application.HelpContext(4010);
end;
