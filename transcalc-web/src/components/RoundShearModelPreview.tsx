...
function RoundShear3D({ params, us }: { params: Record<string, number>; us?: boolean }) {
  ...
  const model = useMemo(() => {
    ...
    g.add(new THREE.ArrowHelper(
      ...
      arrowLen, 0x1f2f3f, Math.min(0.11, arrowLen * 0.30), Math.min(0.08, arrowLen * 0.22),
    ))
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '0', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <input type="checkbox" id="rndshr-dims" checked={showDimensions} onChange={e => setShowDimensions(e.target.checked)} style={{ margin: 0 }} />
        <label htmlFor="rndshr-dims" style={{ cursor: 'pointer', margin: 0, fontWeight: 500 }}>Dimensions</label>
      </div>
    </div>
  )
}
