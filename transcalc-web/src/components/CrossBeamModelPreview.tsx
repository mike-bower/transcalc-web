...
function makeShearGageGroup(size: number, gPad: number): THREE.Group {
  // Local frame: carrier flat in XZ plane, +Y = face normal; ±45° chevron grid
  const grp = new THREE.Group()
  const gridH     = gPad * 0.85
...
function buildCrossBeamScene(p: Props, showDims: boolean, showForces: boolean): THREE.Group {
  ...
  const B     = clamp(p.beamWidthMm * S, 0.05, R_o * 0.55)     // arm width (tangential)
  const T     = clamp(p.beamThicknessMm * S, 0.02, R_o * 0.35) // arm thickness (scene Y = sensor Z)
  const gD    = clamp(p.gageDistFromOuterRingMm * S, 0, (R_o - R_i) * 0.85)
  const armLen = R_o - R_i
  const armMidR = R_i + armLen * 0.5
  const gageR  = R_o - gD  // radial scene distance from center to gage center
  ...
  // -- Outer ring: rectangular cross-section annulus (arms connect at inner face R_o)
  const ringWall = clamp(B * 0.65, 0.03, 0.14)  // radial wall thickness
  const ringShape = new THREE.Shape()
  ringShape.absarc(0, 0, R_o + ringWall, 0, Math.PI * 2, false)
  const ringHole = new THREE.Path()
  ringHole.absarc(0, 0, R_o, 0, Math.PI * 2, true)
  ringShape.holes.push(ringHole)
  const ringGeom = new THREE.ExtrudeGeometry(ringShape, { depth: T, bevelEnabled: false, curveSegments: 56 })
  const ring = new THREE.Mesh(ringGeom, ringMat)
  ring.rotation.x = -Math.PI / 2  // lay shape (XY) into XZ plane
  ring.position.y = -T / 2        // centre vertically
  g.add(ring)

  // -- Hub cylinder
  g.add(new THREE.Mesh(new THREE.CylinderGeometry(R_i, R_i, T * 1.15, 32), hubMat))

  // -- 4 beam arms (XZ plane, Y = up = sensor axial)
  const armX = new THREE.BoxGeometry(armLen, T, B)
  const armZ = new THREE.BoxGeometry(B, T, armLen)
  for (const [xOff, zOff, geom] of [
    [armMidR, 0, armX], [-armMidR, 0, armX],
    [0, armMidR, armZ], [0, -armMidR, armZ],
  ] as [number, number, THREE.BufferGeometry][]) {
    g.add(new THREE.Mesh(geom, beamMat).translateX(xOff).translateZ(zOff))
  }

  // -- Bending gages (linear, amber): top + bottom of all 4 arms at gageR, sensing axis = arm axis
  const gLen = clamp(armLen * 0.22, 0.018, 0.12)
  const gW   = clamp(B * 0.62, 0.016, 0.090)
  const gPad = 0.006

  // [xPos, zPos, rotY]: rotY aligns local +X with arm axis; tabs at local -X point toward hub
  const bendArmConfigs: [number, number, number][] = [
    [ gageR,  0,       0],
    [-gageR,  0,       Math.PI],
    [ 0,      gageR,  -Math.PI / 2],
    [ 0,     -gageR,   Math.PI / 2],
  ]
  for (const [xr, zr, rotY] of bendArmConfigs) {
    for (const ySign of [1, -1]) {
      const pivot = new THREE.Group()
      pivot.rotation.y = rotY
      pivot.position.set(xr, ySign * (T / 2 + gPad * 0.5 + 0.001), zr)
  ...
  // -- Shear gages (±45° chevron, teal): top + bottom of all 4 arms at mid-arm
  const sSize = clamp(B * 0.72, 0.022, 0.10)

  const shearArmConfigs: [number, number, number][] = [
    [ armMidR,  0,       0],
    [-armMidR,  0,       Math.PI],
    [ 0,        armMidR, -Math.PI / 2],
    [ 0,       -armMidR,  Math.PI / 2],
  ]
  for (const [xr, zr, rotY] of shearArmConfigs) {
    for (const ySign of [1, -1]) {
      const pivot = new THREE.Group()
      pivot.rotation.y = rotY
      pivot.position.set(xr, ySign * (T / 2 + gPad * 0.5 + 0.001), zr)
  ...
  // -- Fz load arrow (downward into sensor top face)
  ...
  if (showForces) {
    ...
    // Fy arrow (+Z direction)
    g.add(new THREE.ArrowHelper(
      new THREE.Vector3(0, 0, 1),
      new THREE.Vector3(0, T / 2 + arrowLen * 0.3, -fSmall * 0.5),
      fSmall, 0xe05530, fSmall * 0.24, fSmall * 0.16,
    ))
    // Mz torque arc (8 hatch lines radiating outward from outer ring edge)
    const hatchMat2 = new THREE.LineBasicMaterial({ color: 0x22c55e })
    const nHatch = 8
    for (let i = 0; i < nHatch; i++) {
      const a = (i / nHatch) * Math.PI * 2
      const rx = Math.cos(a) * (R_o + ringWall)
      const rz = Math.sin(a) * (R_o + ringWall)
...
function CrossBeam3D(p: Props) {
  const hostRef = useRef<HTMLDivElement | null>(null)
  const [showDims, setShowDims] = useState(true)
  const [showForces, setShowForces] = useState(true)
  const model = useMemo(() => buildCrossBeamScene(p, showDims, showForces), [p, showDims, showForces])
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: 0, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <span style={{ display: 'inline-block', width: 8, height: 8, background: '#f07030', borderRadius: 1, marginRight: 4 }} />bending gages
        {'  '}
        <span style={{ display: 'inline-block', width: 8, height: 8, background: '#20a880', borderRadius: 1, marginRight: 4, marginLeft: 8 }} />shear gages (45°)
      </div>
    </div>
  )
}
