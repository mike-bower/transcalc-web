 ...
 *
 * Same 45° gage layout as the solid shaft but with a visible bore.
...
function RoundHollowTorque3D({ params, us, materialId }: { params: Record<string, number>; us?: boolean; materialId?: string }) {
  ...
  const model = useMemo(() => {
    ...
    const Ro = clamp((odMm / 2) * mmToScene, 0.06, 0.50)
    ...
    const boreMat = new THREE.MeshStandardMaterial({ color: 0x2a4f6a, roughness: 0.6, metalness: 0.05, side: THREE.BackSide })
    const inner = new THREE.Mesh(new THREE.CylinderGeometry(Ri, Ri, L, 32), boreMat)
    ...
    for (const sign of [-1, 1]) {
      ...
      cap.rotation.y = Math.PI / 2; cap.position.set(sign * L / 2, 0, 0); g.add(cap)
    }
    ...
    addDimensionLine(dim,
      new THREE.Vector3(0, -Ro, Ro + 0.14), new THREE.Vector3(0, Ro, Ro + 0.14),
      `OD=${fmt(odMm)} ${uLabel}`, new THREE.Vector3(0, 0, 1),
    )
    addDimensionLine(dim,
      new THREE.Vector3(L / 2 - 0.1, -Ri, Ro + 0.26), new THREE.Vector3(L / 2 - 0.1, Ri, Ro + 0.26),
      `ID=${fmt(idMm)} ${uLabel}`, new THREE.Vector3(0, 0, 1),
    )
