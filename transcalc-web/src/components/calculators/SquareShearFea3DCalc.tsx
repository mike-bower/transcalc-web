...
interface Props {
  ...
  thicknessMm:  number  // section thickness/height (mm)
...
export default function SquareShearFea3DCalc({ loadN, widthMm, thicknessMm, beamLengthMm, modulusGPa, nu }: Props) {
  ...
  // Max shear at neutral axis for rectangular section: τ = 1.5 * V / A
  const tau_analytical  = 1.5 * loadN / (W * H)
  const gamma_analytical = tau_analytical / G
  const exy_analytical  = gamma_analytical / 2  // tensor shear = γ/2
  ...
  const feaMaxExy = (() => {
    ...
    for (let n = 0; n < mesh.nodeCount; n++) {
      ...
      if (nx > L * 0.3 && nx < L * 0.7 && nz > H * 0.35 && nz < H * 0.65) {
