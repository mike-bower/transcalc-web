...
interface Props {
  ...
  diameterMm: number  // shaft diameter (mm)
...
const DENSITY_OPTIONS: { key: MeshDensity; label: string; nx: number; ny: number; nz: number }[] = [
  { key: 'coarse', label: 'Coarse (4×4×4)',   nx: 4,  ny: 4, nz: 4 },
  { key: 'medium', label: 'Medium (8×6×6)',   nx: 8,  ny: 6, nz: 6 },
  { key: 'fine',   label: 'Fine (14×8×8)',    nx: 14, ny: 8, nz: 8 },
]
...
export default function RoundSolidTorqueFea3DCalc({ torqueNm, diameterMm, lengthMm, modulusGPa, nu }: Props) {
  ...
  // Peak combined shear at xL face surface
  const feaMaxShear = (() => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <p className="workspace-note" style={{ fontSize: '0.75rem', marginTop: 0, marginBottom: -4 }}>
        Round solid shaft — pure torsion load. Fixed: x=0 face. Distributed torque at x=L face. Round cross-section approximated with staircase mesh.
      </p>
