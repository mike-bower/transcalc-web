...
interface Props {
  ...
  beamLengthMm:        number  // total beam length (mm)
  distBetweenGagesMm:  number  // distance between gages D (mm)
  widthMm:             number  // beam width (mm)
  thicknessMm:         number  // beam thickness (mm)
  modulusGPa:          number
  nu:                  number
}
...
export default function ReverseBeamFea3DCalc({ loadN, beamLengthMm, distBetweenGagesMm, widthMm, thicknessMm, modulusGPa, nu }: Props) {
  const [density, setDensity] = useState<MeshDensity>('medium')
  const { solve, solving, progress, solved, error, reset } = useFeaSolver()

  // Equivalent cantilever from support to gage CL
  const L_eff = (beamLengthMm - distBetweenGagesMm) / 2 / 1000  // m
  const F_eff = loadN / 2
  ...
  const I = W * H ** 3 / 12
  const exxAnalytical   = 6 * F_eff * L_eff / (E * W * H ** 2)
  const deltaAnalytical = F_eff * L_eff ** 3 / (3 * E * I)

  function handleSolve() {
    const opt = DENSITY_OPTIONS.find(o => o.key === density)!
    solve({
      meshParams: { L: L_eff, W, H, nx: opt.nx, ny: opt.ny, nz: opt.nz, maskType: 'none' },
      solverParams: { E, nu, fixedGroup: 'x0', loadGroup: 'xL', loadVector: [0, -F_eff, 0] },
    })
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <p className="workspace-note" style={{ fontSize: '0.75rem', marginTop: 0, marginBottom: -4 }}>
        Model: equivalent cantilever from support to gage CL (L=(L−D)/2, F=P/2). Strain magnitude matches analytical gage strain exactly.
      </p>
