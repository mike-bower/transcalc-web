...
function RoundHollowColumn3D({ params, us }: { params: Record<string, number>; us?: boolean }) {
  ...
  const model = useMemo(() => {
    ...
    lblC.position.set(gageLen * 0.6, 0, Ro + 0.16)
    ...
    lblD.position.set(gageLen * 0.6, 0, -Ro - 0.16)
    ...
    addDimensionLine(dim,
      new THREE.Vector3(Ro + 0.18, -H / 2, 0),
      new THREE.Vector3(Ro + 0.18,  H / 2, 0),
      hLabel,
      new THREE.Vector3(1, 0, 0),
    )

    // Outer diameter (horizontal, at front)
    addDimensionLine(dim,
      new THREE.Vector3(-Ro, 0, Ro + 0.14),
      new THREE.Vector3( Ro, 0, Ro + 0.14),
      `OD=${fmt(odMm)} ${uLabel}`,
      new THREE.Vector3(0, 0, 1),
    )

    // Inner diameter (horizontal, offset further out)
    addDimensionLine(dim,
      new THREE.Vector3(-Ri, 0, Ro + 0.26),
      new THREE.Vector3( Ri, 0, Ro + 0.26),
      `ID=${fmt(idMm)} ${uLabel}`,
      new THREE.Vector3(0, 0, 1),
    )
